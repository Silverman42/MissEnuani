<template>
    <div>
        ...
        <div class="mb-3 grid grid-cols-2 col-gap-2">
            <p
                class="col-start-1 col-end-3 text-xs flex w-full justify-between"
            >
                <span>Height (ft)</span>
                <span class="text-xs italic text-gray-600">Required</span>
            </p>
            <text-input type="number" form="medicals" v-model="height.number" />
            <text-input
                type="number"
                form="medicals"
                v-model="height.decimal"
            />
            <p
                v-if="$page.errors.height"
                class="col-start-1 col-end-3 text-xs text-red-500"
            >
                {{ $page.errors.height }}
            </p>
        </div>
        <div class="mb-3 grid grid-cols-2 col-gap-2">
            ...
            <text-input
                type="number"
                form="medicals"
                v-model="weight.decimal"
            />
            <p
                v-if="$page.errors.weight"
                class="col-start-1 col-end-3 text-xs text-red-500"
            >
                {{ $page.errors.weight }}
            </p>
        ...
        <div class="mb-3 grid grid-cols-3 col-gap-2">
            ...
            <div
                class="col-start-3 col-end-4 rounded-md"
                :style="{ background: colorMatch[skinColor] }"
            ></div>
        </div>
        <div class="mb-3">
            <select-input
                label="Blood Type"
                :error="$page.errors.blood_type"
                :value="bloodType"
                v-model="bloodType"
                form="medicals"
            >
        ...
        <div class="mb-3">
            ...
            <div class="flex flex-wrap">
                <radio-input
                    value="1"
                    name="canSmoke"
                    @input="changeSmokeStatus($event)"
                    :defaultValue="canSmoke"
                    title="Yes"
                />
                <radio-input
                    value="0"
            ...
            <div v-if="$page.errors.can_smoke" class="mt-2">
                {{ $page.errors.can_smoke || "" }}
            </div>
        </div>
        <div class="mb-3">
            ...
            <div class="flex flex-wrap">
                <radio-input
                    value="1"
                    name="isVege"
                    @input="changeAllergiestatus($event)"
                ...
                <radio-input
                    value="0"
                    name="isVege"
                    @input="changeVegeStatus($event)"
                    :defaultValue="isVegetarian"
                    title="No"
                />
            </div>
            <div v-if="$page.errors.is_vegetarian" class="mt-2">
                {{ $page.errors.is_vegetarian || "" }}
            </div>
        </div>
        <div class="mb-3">
            <textarea-input
                v-model="dietaryNeeds"
                :error="$page.errors.dietary_needs"
                form="medicals"
        ...
        <div class="mb-3">
            <textarea-input
                v-model="recentSickness"
                :error="$page.errors.recent_sickness"
                form="medicals"
...
export default {
    ...
    methods: {
        ...
        submitForm() {
            ...
            form.append(
                "height",
                `${this.height.number}.${this.height.decimal}`
            );
            form.append(
                "weight",
                `${this.weight.number}.${this.weight.decimal}`
            );
            ...
            this.$inertia.post(
                this.$route.relativePath("client.profile.update.medicals"),
                form
            );
        },
    },
