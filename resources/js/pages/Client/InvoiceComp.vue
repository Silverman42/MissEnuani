<template>
    <div class="flex flex-wrap">
        <div class="w-full md:w-6/12 mb-6 md:mb-10 md:pr-2">
            <h6 class="text-2xl font-bold capitalize">
        ...
        <div class="w-full md:w-6/12 md:text-right md:mb-10 mb-6 md:pr-2">
            ...
            <p class="text-gray-900 text-3xl">
                <sup class="text-xs text-yellow-600">{{ getCurrency }} </sup
                >{{ invoice.amount | changeNumberFormat
                }}<sup class="text-xs text-yellow-600"> .00</sup>
            </p>
        </div>
        <div class="w-full md:w-4/12 mb-6 md:pr-2">
            <h6 class="uppercase text-xs tracking-widest">Payment Status</h6>
            <p
                class="inline-block w-auto px-4 py-1 rounded-md"
                :class="{
                    'bg-green-300': paymentWasCompleted === true,
                    'bg-red-300': paymentWasCompleted === false,
                }"
            >
                {{ paymentWasCompleted === true ? "Paid" : "Unpaid" }}
            </p>
        </div>
        <div class="w-full md:w-4/12 mb-6 md:pr-2">
