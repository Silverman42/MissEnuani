<template>
    <div>
        ...
        <section class="flex flex-wrap md:flex-no-wrap justify-between">
            ...
            <div
                class="w-full md:flex-grow px-2 py-8 md:p-12 rounded-md bg-white shadow-sm reveal-left"
                style="--delay: 0.3s"
            >
                    ...
                    <template v-slot:tab-body-user-presence>
                        <section v-if="$page.user.is_active == 1" class="pt-8">
                            ...
                            <div class="mt-8 flex justify-end">
                                <secondary-btn
                                    color="red"
                                    @click="deactivateUser($page.user.id)"
                                    extraClass="w-full md:w-auto"
                                    >Deactivate User</secondary-btn
                                >
                            </div>
                        </section>
                        <section v-if="$page.user.is_active == 0" class="pt-8">
                            ...
                            <div class="mt-8 flex justify-end">
                                <secondary-btn
                                    color="primary"
                                    @click="activateUser($page.user.id)"
                                    extraClass="w-full md:w-auto"
                                    >Activate User</secondary-btn
                                >
                            </div>
                        </section>
                        <section class="pt-12">
                            ...
                            <div class="mt-8 flex justify-end">
                                <secondary-btn
                                    color="red"
                                    @click="removeUser($page.user.id)"
                                    extraClass="w-full md:w-auto"
                                    >Remove User</secondary-btn
                                >
                            </div>
                        </section>
                    </template>
...
export default {
    ...
    components: {
        ...
        UpdateUser: () =>
            import(/* webpackChunkName: "user_page" */ "./UserModify"),
    },
    mounted() {
        const permissions = this.$page.user;
