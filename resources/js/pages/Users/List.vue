    ...
    <div>
        ...
        <modal :isOpen.sync="modalIsOpen">
            <div class="mb-10 reveal-left" style="--delay: 0.1s">
                <h3 class="text-lg font-extrabold">Create New Users</h3>
                <p class="text-sm">
                    Create user biodata, avatar, password and permissions
                </p>
            </div>
            <create-user
                :competitionYears="$page.competition"
        ...
        >
            ...
            <div class="w-full md:w-auto mt-3 md:mt-0 border">
                <primary-btn width="w-full md:w-auto" @click="openModal"
                    >Create New User</primary-btn
                >
            </div>
        </section>
        <section
            class="rounded-md bg-white shadow-sm px-2 py-8 md:p-12 reveal-up"
            style="--delay: 0.5s"
        >
                ...
                <dropdown>
                    ...
                    <template slot="box">
                        <div class="inline-flex flex-col items-start">
                            <inertia-link
                                :href="$route.relativePath('users.recycle')"
            ...
            <template v-if="getUsers.length > 0">
                <div class="flex flex-wrap justify-center mb-5">
                    <user-list
                        ...
                        :email="user.email"
                        :viewLink="
                            $route.relativePath('users.show', user.id || 0)
                        "
                ...
                <div class="flex items-center justify-end">
                    <secondary-btn
                        ...
                        extraClass="mr-2"
                        @click="paginate(currentPageNumber - 1)"
                        >Previous</secondary-btn
                    >
                    <secondary-btn
                        v-if="nextLink !== null"
                        @click="paginate(currentPageNumber + 1)"
...
export default {
    ...
    components: {
        Modal: () =>
            import(
                /* webpackChunkName: "user_page" */ "../../components/Modal"
            ),
        UserList: () =>
            import(
                /* webpackChunkName: "user_page" */ "../../components/UserList"
            ),
        CreateUser: () =>
            import(/* webpackChunkName: "user_page" */ "./CreateUser.vue"),
    },
    data() {
        return {
            modalIsOpen: false,
    ...
    methods: {
        ...
        removeUser(userId) {
            const result = confirm(
                "Are you sure you want to send this user to the recycle bin ?"
            );
            if (result === true) {
                this.$inertia.post(
                    this.$route.relativePath("users.remove", userId)
                );
            }
        },
        deactivateUser(userId) {
            ...
            if (result === true) {
                this.$inertia.post(
                    this.$route.relativePath("users.deactivate", userId)
                );
            }
        },
        activateUser(userId) {
            ...
            if (result === true) {
                this.$inertia.post(
                    this.$route.relativePath("users.activate", userId)
                );
            }
        },
        searchUsers() {
